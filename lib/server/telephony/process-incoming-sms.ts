...
export async function processIncomingSms(sms: IncomingSms) {
  ...
  const { data: number, error: numberError } = await supabase
    .from("textback_numbers").select("id,business_name")
  ...
  const { data: missedCall, error: callError } = await supabase.from("missed_call_events")
    .select("id").eq("textback_number_id", number.id).eq("caller_number", sms.senderNumber)
  ...
  const { data: conversation, error: conversationError } = await supabase.from("conversations").upsert({
    textback_number_id: number.id, customer_number: sms.senderNumber, status: "new",
    last_message_at: new Date().toISOString(), latest_inbound_preview: sms.message.slice(0, 200),
  ...
  if (missedCall?.id) {
    await supabase.from("missed_call_events").update({
      customer_replied_at: new Date().toISOString(), conversation_id: conversation.id,
    }).eq("id", missedCall.id);
  }

  await finalizeReadySelfServiceNumber(number.id);
  return { status: "stored" as const, messageId: message.id, conversationId: conversation.id, businessName: number.business_name };
}
