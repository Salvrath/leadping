...
describe("Textback application emails", () => {
  ...
  it("escapes unsafe HTML in the customer template", () => {
    const email = customerApplicationEmail({ ...lead, company: '<script>alert("x")</script>' }, "lead-123");
    expect(email.html).not.toContain("<script>");
    expect(email.html).toContain("scriptalert");
  });
});
