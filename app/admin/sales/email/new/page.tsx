...
export default async function NewSalesEmailCampaignPage({ searchParams }: { searchParams?: { error?: string } }) {
  ...
  const [{ data: leads }, { data: suppressions }] = await Promise.all([
    db.from("sales_leads").select("id,company_name,contact_name,contact_role,industry,city,email_address,email_type,email_status,email_verified_at,email_outbound_count,automation_score,fit_score,do_not_contact").eq("email_status", "verified").eq("do_not_contact", false).lt("email_outbound_count", 2).order("automation_score", { ascending: false }).limit(500),
  ...
  const eligible = (leads || []).filter((lead) =>
    ...
    && (lead.email_type === "generic" || Boolean(lead.email_type === "personal" && lead.contact_name && lead.contact_role))
    && !suppressed.has(lead.email_address.toLocaleLowerCase("en-US"))
  );
  ...
  return <main className="admin-page"><div className="admin-wrap sales-narrow-wide">
    ...
    <p className="admin-intro">Verifierade generella företagsadresser och namngivna beslutsfattares arbetsmejl kan väljas. Du får en fullständig förhandsvisning innan något skickas.</p>
    {errorText && <div className="admin-note error">{errorText}</div>}

    {eligible.length === 0 ? <AdminEmpty title="Inga utskicksklara e-postadresser" text="Lägg in en verifierad företagsadress eller en namngiven beslutsfattares arbetsmejl med källa."/> : <form action={createSalesEmailCampaign} className="sales-form">
      ...
      <section className="admin-card admin-section">
        <div className="admin-section-head"><div><h2>Välj mottagare</h2><p>Högst {salesEmailBatchLimit()} företag per kampanj. Personliga adresser visas endast när namn och beslutsroll är verifierade.</p></div><span className="admin-badge active">{eligible.length} tillgängliga</span></div>
        <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Välj</th><th>Företag</th><th>Mottagare</th><th>E-post</th><th>Prioritet</th><th>Kontaktförsök</th></tr></thead><tbody>{eligible.map((lead) => <tr key={lead.id}><td><input type="checkbox" name="lead_id" value={lead.id}/></td><td><strong>{lead.company_name}</strong><div className="muted">{[lead.industry, lead.city].filter(Boolean).join(" · ") || "–"}</div></td><td>{lead.email_type === "personal" ? <><strong>{lead.contact_name}</strong><div className="muted">{lead.contact_role}</div></> : <span>Företagsinkorg</span>}</td><td>{lead.email_address}</td><td>{lead.automation_score || lead.fit_score}/100</td><td>{lead.email_outbound_count} av 2</td></tr>)}</tbody></table></div>
        <div className="admin-mobile-list">{eligible.map((lead) => <label className="admin-mobile-card sales-mobile-select" key={lead.id}><input type="checkbox" name="lead_id" value={lead.id}/><span><strong>{lead.company_name}</strong><small>{lead.email_type === "personal" ? `${lead.contact_name} · ${lead.contact_role}` : "Företagsinkorg"}</small><small>{lead.email_address}</small><small>{[lead.industry, lead.city].filter(Boolean).join(" · ") || "–"} · prioritet {lead.automation_score || lead.fit_score}</small></span></label>)}</div>
      </section>
