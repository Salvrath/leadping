...
export default async function ProviderNumbersPage() {
  ...
  const [{ data: numbers }, { count: waiting }] = await Promise.all([
    db.from("provider_number_inventory").select("id,provider,provider_number,status,configured_at,assigned_at,textback_numbers:assigned_textback_number_id(business_name)").order("created_at", { ascending: false }),
  ...
  return <main style={{minHeight:"100vh",background:"#f4f7fb",padding:"28px clamp(16px,5vw,72px)",color:"#10213f"}}>
    <div style={{maxWidth:1050,margin:"0 auto"}}><Link href="/admin">← Till driftpanelen</Link>
      <section style={{background:"white",border:"1px solid #dbe4ef",borderRadius:18,padding:28,marginTop:20}}>
        ...
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr><th align="left">Nummer</th><th align="left">Status</th><th align="left">Tilldelat företag</th><th align="left">Konfigurerat</th><th>Åtgärd</th></tr></thead><tbody>{(numbers || []).map((number: any) => { const assigned = Array.isArray(number.textback_numbers) ? number.textback_numbers[0] : number.textback_numbers; return <tr key={number.id} style={{borderTop:"1px solid #e2e8f0"}}><td style={{padding:"12px 4px"}}>{number.provider_number}</td><td>{number.status}</td><td>{assigned?.business_name || "–"}</td><td>{number.configured_at ? new Intl.DateTimeFormat("sv-SE",{dateStyle:"short",timeStyle:"short"}).format(new Date(number.configured_at)) : "–"}</td><td align="center">{number.status === "available" && <form action={disableProviderNumber}><input type="hidden" name="id" value={number.id}/><button>Inaktivera</button></form>}</td></tr>; })}</tbody></table></div>
      </section>
    </div>
  </main>;
}
