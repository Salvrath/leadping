...
export default async function AdminPage() {
  ...
  const [{ data: numbers }, { data: conversations }, { data: calls }] = await Promise.all([
  ...
  const stats = {
    ...
    failures: allCalls.filter((item) => ["sms_retry_pending", "sms_dead_letter"].includes(item.status)).length,
  };
  ...
  return <main style={{minHeight:"100vh",background:"#f4f7fb",color:"#10213f",padding:"24px clamp(16px,4vw,56px) 56px"}}>
    <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:20,marginBottom:28}}>
      ...
      <form action={logoutAdmin}><button style={{border:"1px solid #cbd5e1",background:"white",padding:"10px 14px",borderRadius:10,cursor:"pointer"}}>Logga ut</button></form>
    </header>
    ...
    <section style={{...panel,marginBottom:24,overflowX:"auto"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",minWidth:720}}><thead><tr>{['Företag','Textback-nummer','Ordinarie nummer','Leverantör','Status','Åtgärd'].map(h=><th key={h} style={{textAlign:"left",padding:"10px 8px",borderBottom:"1px solid #e2e8f0",fontSize:13,color:"#64748b"}}>{h}</th>)}</tr></thead><tbody>
        {(numbers || []).map(number=><tr key={number.id}><td style={{padding:8,fontWeight:700}}>{number.business_name}</td><td style={{padding:8}}>{number.provider_number}</td><td style={{padding:8}}>{number.business_phone_numbers?.join(', ') || '–'}</td><td style={{padding:8}}>{number.provider}</td><td style={{padding:8}}><span style={pill(number.active?'active':'inactive')}>{number.active?'Aktiv':'Pausad'}</span></td><td style={{padding:8}}><form action={setTextbackNumberActive}><input type="hidden" name="id" value={number.id}/><input type="hidden" name="active" value={String(!number.active)}/><button style={{border:"1px solid #cbd5e1",background:"white",borderRadius:8,padding:"7px 10px",cursor:"pointer"}}>{number.active?'Pausa':'Aktivera'}</button></form></td></tr>)}
      </tbody></table>
    </section>
