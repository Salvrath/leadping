...
export default async function OnboardingPage() {
  ...
  const [{ data: number }, { data: lead }] = await Promise.all([
    ...
    db.from("pilot_leads").select("telephony,workshop_phone,provisioning_status,provisioning_error").eq("textback_number_id", user.textback_number_id).maybeSingle(),
  ]);
  ...
  const billingAttention = lead?.provisioning_status === "billing_attention";
  ...
  return <main style={{minHeight:"100vh",background:"#f4f7fb",padding:"28px clamp(16px,5vw,72px)",color:"#10213f"}}>
    <div style={{maxWidth:980,margin:"0 auto"}}>
      ...
      <section style={{background:"white",border:"1px solid #dbe4ef",borderRadius:18,padding:28}}>
        {number.active ? <><CheckCircle2 size={48}/><span style={{display:"inline-block",marginTop:12,fontWeight:800,color:"#166534"}}>AKTIVERAD</span><h1>{number.business_name} är anslutet</h1><p>Textback är i live-läge och abonnemanget har startats. Missade samtal kan nu följas upp automatiskt och kundsvar samlas i portalen.</p><Link className="button large" href="/portal">Öppna kundportalen</Link></> : <>
          <span style={{fontWeight:800,color:"#1976d2"}}>STEG {done} AV 6 KLARA</span><h1>Anslut företagets telefoni</h1><p>Textback-numret <strong>{number.provider_number}</strong> är reserverat för {number.business_name}. När samtliga test är godkända startas abonnemanget och tjänsten aktiveras automatiskt.</p>
          {billingAttention && <p role="alert" style={{background:"#fff1f2",color:"#9f1239",padding:14,borderRadius:12}}>Telefonitesterna är klara men Stripe kunde inte starta abonnemanget automatiskt. Ingen tjänst har aktiverats. Kontakta <a href="mailto:info@textback.se">info@textback.se</a> och ange företagsnamnet.</p>}
