...
export async function sendCustomerReply(formData: FormData) {
  ...
  const db = getSupabaseAdmin();
  const { data: existing, error: existingError } = await db.from("sms_messages")
    .select("id").eq("client_request_id", requestId).maybeSingle();
  ...
  try {
    ...
    await db.from("sms_messages").update({
      ...
      sms_parts: result.parts || null,
      sms_cost: result.cost || null,
      sent_at: now,
